  ...
  if (active && payload?.length) {
    return (
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
        ...
        <p style={{ color: 'var(--accent2)' }}>₹{(payload[0].value || 0).toFixed(2)}</p>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid-4 mb-16">
            {[
              { label: "Net Revenue", value: `₹${(report.totalNetRevenue || 0).toFixed(2)}`, color: 'var(--green)' },
              { label: "Offers Applied", value: `₹${(report.totalDiscounts || 0).toFixed(2)}`, color: 'var(--yellow)' },
              { label: "Gross Revenue", value: `₹${(report.totalGrossRevenue || 0).toFixed(2)}`, color: 'var(--cyan2)' },
              { label: "Bills Created", value: report.totalBills || 0, color: 'var(--accent2)' },
            ].map((s, i) => (
          ...
          <div className="grid-2 mb-16">
            <motion.div className="card" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
              ...
              <div className="total-row"><span style={{ color: 'var(--text3)' }}>📸 Polaroids</span><span style={{ fontWeight: 700 }}>₹{(report.breakdown?.polaroid || 0).toFixed(2)}</span></div>
              <div className="total-row"><span style={{ color: 'var(--text3)' }}>📱 Digital Photos</span><span style={{ fontWeight: 700 }}>₹{(report.breakdown?.digitalPhoto || 0).toFixed(2)}</span></div>
              <div className="total-row"><span style={{ color: 'var(--text3)' }}>🖼️ Posters</span><span style={{ fontWeight: 700 }}>₹{(report.breakdown?.poster || 0).toFixed(2)}</span></div>
              <div className="total-row"><span style={{ color: 'var(--text3)' }}>🔖 Stickers</span><span style={{ fontWeight: 700 }}>₹{(report.breakdown?.sticker || 0).toFixed(2)}</span></div>
            ...
            <h3 style={{ marginBottom: 16, fontWeight: 700 }}>Payment Methods</h3>
              {!report.qrBreakdown || report.qrBreakdown?.filter(q => q.total > 0).length === 0
                ...
                : report.qrBreakdown?.map((qr) => (
                  <div key={qr.qr} style={{ marginBottom: 10 }}>
                    <div className="total-row" style={{ marginBottom: 4 }}>
                      ...
                      <span style={{ fontWeight: 700 }}>₹{(qr.total || 0).toFixed(2)} <span style={{ fontWeight: 400, fontSize: 12, color: 'var(--text3)' }}>({qr.count})</span></span>
                    </div>
                  ...
                  <tbody>
                    {report.topItems.map((item, idx) => (
                      <tr key={item.name}>
                        ...
                        <td>{item.qty}</td>
                        <td style={{ fontWeight: 700, color: 'var(--accent2)' }}>₹{(item.revenue || 0).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
